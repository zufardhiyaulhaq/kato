...
kato ships **19 read-only checks** you compose into flows — pods, workloads, nodes,
networking, and config:

| Area | Methods |
|---|---|
| Pods | `check_pod_status`, `check_pod_logs`, `describe_pod`, `check_pod_resources`, `check_pod_usage` |
| Workloads | `check_deployment_status`, `describe_deployment`, `check_replicaset`, `check_daemonset_status`, `check_hpa` |
