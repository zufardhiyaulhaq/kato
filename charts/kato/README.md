...
- **`UseCase`** is the flow: inputs, ordered steps, `when` conditions, `forEach`
  fan-out, per-step `summaryFilter`, and the summary prompt.
- **`ModelConfig`** is the LLM backend (OpenAI-compatible). UseCases pick one via
  `summary.modelConfigRef` or fall back to the default.
- **`Run`** is the audit record of each execution: inputs, per-step outputs, and the
...
Full reference and every output field: [`docs/METHOD.md`](https://github.com/gopaytech/kato/blob/main/docs/METHOD.md).
...
live under [`examples/`](https://github.com/gopaytech/kato/tree/main/examples).
...
[`examples/modelconfig/`](https://github.com/gopaytech/kato/tree/main/examples/modelconfig).
...
```console
helm repo add kato https://gopaytech.github.io/kato/
...
see example files [here](https://github.com/gopaytech/kato/blob/main/charts/kato/values.yaml)
